...
public static partial class OptimizedEnumerable
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool All<TSource>(this ObservableCollection<TSource> source, Func<TSource, bool> predicate)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool All<TSource>(this ObservableCollection<TSource> source, int start, int length, Func<TSource, bool> predicate)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool All<TSource, TState>(this ObservableCollection<TSource> source, TState state, Func<TSource, TState, bool> predicate)
    {
        for (var i = 0; i < source.Count; i++)
        {
            if (!predicate(source[i], state))
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool All<TSource, TState>(this ObservableCollection<TSource> source, int start, int length, TState state, Func<TSource, TState, bool> predicate)
    {
        var last = start + length;
        for (var i = start; i < last; i++)
        {
